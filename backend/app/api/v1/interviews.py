...
async def get_interview_stats(
    ...
    cache_key = f"interviews-dashboard-stats:{current_user.get('company_id', '')}:{current_user.get('id', '')}:{days or 0}"
    ...
    stats = await InterviewService.get_dashboard_stats(db, current_user, start_date=start_date)
