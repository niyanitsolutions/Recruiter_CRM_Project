...
const TopBar = ({ title, subtitle, actions }) => {
  ...
  return (
    <header className="h-16 bg-surface-50/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 sticky top-0 z-30">
      <div>
        {title && <h1 className="text-xl font-semibold text-surface-800">{title}</h1>}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative hidden md:block">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-64 pl-10 pr-4 py-2 text-sm bg-surface-100 border border-surface-200/50 rounded-lg text-surface-700 placeholder:text-surface-400 focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20 transition-all"
          />
        </div>
        ...
        <div className="relative" ref={notificationsRef}>
          ...
          {isNotificationsOpen && (
            <div className="dropdown right-0 w-80 animate-slide-up">
              ...
              <div className="px-4 py-2 border-t border-surface-200/30">
                <button
          ...
          {isProfileOpen && (
            <div className="dropdown right-0 animate-slide-up">
              <div className="px-4 py-3 border-b border-surface-200/30">
                ...
                {user?.companyName && (
                  <p className="text-xs text-accent-400 mt-1">{user.companyName}</p>
                )}
              </div>
              <div className="py-1">
              ...
              <div className="border-t border-surface-200/30 py-1">
                <button
                  onClick={handleLogout}
                  className="dropdown-item flex items-center gap-3 text-red-400 hover:bg-red-500/10"
                >
