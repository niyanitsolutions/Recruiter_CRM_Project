...
const ClientImportModal = ({ onClose, onImported }) => {
  ...
  const handlePreview = async () => {
    ...
    setLoading(true)
    try {
      const data = await clientService.bulkImportPreview(file)
      setPreview(data)
      setStep('preview')
    } catch (err) {
  ...
  const handleImport = async () => {
    ...
    setLoading(true)
    try {
      const data = await clientService.bulkImport(file)
      setResults(data)
      setStep('results')
                ...
                <table className="w-full text-sm">
                  <thead className="bg-surface-50 border-b border-surface-200">
                    <tr>
                      ...
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Company Name</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Type</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Industry</th>
                      ...
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Phone</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Contact Person</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Contact Email</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Commission %</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Status</th>
                      <th className="text-left px-3 py-2 font-medium text-surface-600 whitespace-nowrap">Row Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-surface-100">
                    {preview.rows.map((row) => (
                      <tr key={row.row} className={rowBg(row)}>
                        ...
                        <td className={`px-3 py-2 font-medium ${!row.fields.name ? 'text-red-600' : 'text-surface-900'}`}>
                          {row.fields.name || 'MISSING'}
                        </td>
                        <td className="px-3 py-2 text-surface-600 capitalize">
                          {row.fields.client_type || '—'}
                        </td>
                        ...
                        <td className="px-3 py-2 text-surface-600 text-xs font-mono">
                          {row.fields.email || '—'}
                        </td>
                        <td className="px-3 py-2 text-surface-600 text-xs font-mono">
                          {row.fields.phone || '—'}
                        ...
                        <td className="px-3 py-2 text-surface-600">
                          {row.fields.contact_name || '—'}
                        </td>
                        <td className="px-3 py-2 text-surface-600 text-xs font-mono">
                          {row.fields.contact_email || '—'}
                        </td>
                        <td className="px-3 py-2 text-surface-600 text-right">
                          {row.fields.commission_percentage || '—'}
                        </td>
                        <td className="px-3 py-2 text-surface-600 capitalize">
