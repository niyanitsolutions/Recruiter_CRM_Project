...
function GenerateModal({ templateId, onClose, onDone }) {
  ...
  // Dynamic form: only placeholders actually used by the selected template's body.
  ...
  useEffect(() => {
    ...
    if (t) {
      ...
      setEmailSubject(t.name)
      setEmailMessage(defaultEmailMessage(t.name))
    }
  }, [tmplId, templates])

  // Re-detect placeholders + re-resolve their values every time the template
  // or recipient changes — the same template is reusable for anyone.
  useEffect(() => {
    if (!tmplId) { setPlaceholders([]); setFieldValues({}); return }
    setContextLoading(true)
    const params = recipientType === 'employee'
      ? (empId ? { employee_id: empId } : {})
      : (candId ? { candidate_id: candId } : {})
    documentCenterService.getGenerateContext(tmplId, params)
      .then(r => {
  ...
  }, [tmplId, recipientType, empId, candId])
      ...
      <div className="rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]" style={{ background: 'var(--bg-card)' }}>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          {/* Dynamic fields — one editable input per placeholder actually used
              by the selected template. Auto-filled from the selected recipient;
              edits here affect only this generated document. */}
                ...
                {placeholders.map(token => (
                  <div key={token} className="col-span-1">
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-violet-500"
                      style={{ background: 'var(--bg-card-alt)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}
                    />
                  </div>
                ))}
              ...
              <div className="mt-3 space-y-2.5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full px-2.5 py-1.5 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-violet-500 resize-y"
                    style={{ background: 'var(--bg-card-alt)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}
                  />
                </div>
              </div>
