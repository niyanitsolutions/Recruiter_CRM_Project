...
// A template "looks like" a recruitment/hiring document when its name/category
// hints at candidate-facing paperwork (offer letters etc.) — used only to pick
// a sensible default tab; the user can always switch to the other recipient type.
...
// Friendly labels for the placeholder-driven Generate form. Any token not
// listed here (a future/custom placeholder) still gets a field — just with an
// auto-prettified label — so nothing needs to be hardcoded to "support" it.
const PLACEHOLDER_LABELS = {
  employee_name: 'Employee Name', employee_id: 'Employee ID', employee_code: 'Employee Code',
  department: 'Department', designation: 'Designation', employment_type: 'Employment Type',
  employee_status: 'Employee Status', joining_date: 'Joining Date', exit_date: 'Exit Date',
  probation_end_date: 'Probation End Date', salary: 'Salary', manager_name: 'Reporting Manager',
  employee_email: 'Employee Email', employee_phone: 'Employee Phone', employee_address: 'Employee Address',
  employee_gender: 'Gender', employee_dob: 'Date of Birth', work_location: 'Work Location', branch: 'Branch',
  company_name: 'Company Name', company_address: 'Company Address', company_phone: 'Company Phone',
  company_email: 'Company Email', company_website: 'Company Website', gst_number: 'GST Number',
  reg_number: 'Registration Number', current_date: 'Current Date', current_month: 'Current Month',
  month_year: 'Month & Year', current_year: 'Current Year', basic: 'Basic Salary', ctc: 'CTC',
  hra: 'HRA', special_allowance: 'Special Allowance', bonus: 'Bonus', gross: 'Gross Salary',
  pf: 'PF', esi: 'ESI', pt: 'Professional Tax', tds: 'TDS', total_deductions: 'Total Deductions',
  net_salary: 'Net Salary', salary_in_words: 'Salary in Words', remarks: 'Remarks',
  offer_date: 'Offer Date', confirmation_date: 'Confirmation Date', generated_by: 'Generated By',
}
const labelForField = (token) =>
  PLACEHOLDER_LABELS[token] || token.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())

const defaultEmailMessage = (templateName) =>
...
function GenerateModal({ template, onClose, onDone }) {
  ...
  const [recipientType, setRecipientType] = useState(looksLikeRecruitment(template) ? 'candidate' : 'employee')
  ...
  // Dynamic form: only placeholders actually used by this template's body.
  ...
  const [emailSubject, setEmailSubject]   = useState(template.name)
  const [emailMessage, setEmailMessage]   = useState(defaultEmailMessage(template.name))
  ...
  // Re-detect placeholders + re-resolve their values every time the recipient
  // (or recipient type) changes — the same template is reused for anyone.
  useEffect(() => {
    setContextLoading(true)
    const params = recipientType === 'employee'
      ? (empId ? { employee_id: empId } : {})
      : (candId ? { candidate_id: candId } : {})
    documentCenterService.getGenerateContext(templateId, params)
      .then(r => {
        const data = r.data?.data || {}
        setPlaceholders(data.placeholders || [])
        setFieldValues(data.values || {})
        // Only auto-fill the email recipient/message until the user edits them by hand.
        if (!emailTouched) setEmailTo(data.recipient_email || '')
      })
      .catch(() => {})
      .finally(() => setContextLoading(false))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [templateId, recipientType, empId, candId])
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await documentCenterService.generateDocument({
        ...
        recipient_email: sendEmail ? emailTo.trim() : null,
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60">
      <div className="rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]" style={{ background: 'var(--bg-card)' }}>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
              ...
              {[
                ...
                <button
                  ...
                  style={recipientType === opt.key
                    ? { background: 'linear-gradient(135deg, #167CFB, #0267F9)', borderColor: '#167CFB' }
            ...
            {recipientType === 'employee' ? (
              <>
                <input
                  ...
                  className="w-full px-3 py-2 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent-500 mb-1.5"
                  style={{ background: 'var(--bg-card-alt)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}
                />
                <div className="border rounded-lg max-h-44 overflow-y-auto divide-y" style={{ borderColor: 'var(--border)' }}>
                  {empLoading ? (
                      ...
                      {filteredEmps.map(e => (
                        <label key={e._id || e.id}
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            {e.email && (
                              <span className="block text-xs truncate" style={{ color: 'var(--text-muted)' }}>{e.email}</span>
                            )}
                          </span>
                        </label>
                      ))}
                      ...
                      {filteredCands.map(c => (
                        <label key={c._id || c.id}
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            {c.email && (
                              <span className="block text-xs truncate" style={{ color: 'var(--text-muted)' }}>{c.email}</span>
                            )}
                          </span>
                        </label>
                      ))}
          ...
          {/* Dynamic fields — one editable input per placeholder actually used
              by this template. Auto-filled from the selected recipient; edits
              here affect only this generated document, never the master record. */}
          {placeholders.length > 0 && (
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>
                Document Fields {contextLoading && <span className="normal-case font-normal">(refreshing…)</span>}
              </label>
              <div className="grid grid-cols-2 gap-x-3 gap-y-2.5 p-3 rounded-lg border" style={{ borderColor: 'var(--border)' }}>
                {placeholders.map(token => (
                  <div key={token} className="col-span-1">
                    <p className="text-[11px] mb-0.5" style={{ color: 'var(--text-muted)' }}>{labelForField(token)}</p>
                    <input
                      value={fieldValues[token] ?? ''}
                      onChange={e => setFieldValues(v => ({ ...v, [token]: e.target.value }))}
                      className="w-full px-2.5 py-1.5 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent-500"
                      style={{ background: 'var(--bg-card-alt)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}
                    />
                  </div>
                ))}
          ...
          {/* Optional email delivery — reuses the existing email pipeline server-side. */}
          <div className="rounded-lg border p-3" style={{ borderColor: 'var(--border)' }}>
            <label className="flex items-center gap-2 cursor-pointer text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
              <input type="checkbox" checked={sendEmail} onChange={e => setSendEmail(e.target.checked)} className="accent-[#167CFB]" />
              Send generated document by email
            </label>
            {sendEmail && (
              <div className="mt-3 space-y-2.5">
                <div>
                  <p className="text-[11px] mb-0.5" style={{ color: 'var(--text-muted)' }}>Recipient Email</p>
                  <input
                    value={emailTo}
                ...
                <div>
                  <p className="text-[11px] mb-0.5" style={{ color: 'var(--text-muted)' }}>Subject</p>
                  <input
                    value={emailSubject}
                ...
                <div>
                  <p className="text-[11px] mb-0.5" style={{ color: 'var(--text-muted)' }}>Message</p>
                  <textarea
                    value={emailMessage}
                    onChange={e => setEmailMessage(e.target.value)}
                    rows={5}
                    className="w-full px-2.5 py-1.5 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent-500 resize-y"
                    style={{ background: 'var(--bg-card-alt)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}
                  />
                </div>
              </div>
        ...
        <div className="flex gap-3 px-6 pb-6 pt-4 border-t flex-shrink-0" style={{ borderColor: 'var(--border)' }}>
          <button
            onClick={onClose}
